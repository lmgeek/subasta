...
<div class="row" style="margin-top: 22px;margin-left:0px;">
		 ...
		 <div class="">
			@if ($disponible > 0 and $status != \App\Auction::FUTURE)
				...
				<a href="#" data-target="<? if (Auth::user()->type == \App\User::VENDEDOR){ ?> #bid-Modal-danger <? } else {?> #bid-Modal-{{ $a->id }} <? } ?>" data-toggle="modal">
		...
		@else
			<? if (Auth::user()){ ?>
				...
				<a href="#" data-target="<? if (Auth::user()->type == \App\User::VENDEDOR){ ?> #bid-Modal-danger <? } else {?> #bid-Modal-warning <? } ?>" data-toggle="modal">
                    <img alt="image" width="150px"  src="{{ asset('/landing/img/subastar.png') }}" />
                </a>
