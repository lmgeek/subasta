    ...
    <div class="task-listing-details" id="AuctionLeft<?=$auction->id?>">
        ...
        <div class="task-listing-description">
            <h3 class="task-listing-title">
                <a href="{{($auction->timeline!=Constants::FINISHED)?'/subastas/ver/'.$auction->id:'#'}}"
                   id="LinkSubasta{{$auction->id}}">
                ...
                @if($auction->type!='public')
                    <em class="t16 icon-feather-eye-off" data-tippy-placement="right" title="Subasta Privada"
                        data-tippy-theme="dark"></em>
                @endif
            </h3>
            <ul class="task-icons">
                @if(isset(Auth::user()->id) and Auth::user()->id==$auction->batch->arrive->boat->user_id)
                    <li>
                        <em class="icon-material-outline-gavel <?=(empty($finished) ? 'primary' : '')?>"></em> {{$auction->code}}
                    </li>
                @endif
                <li><em class="icon-material-outline-access-time <?=(empty($finished) ? 'primary' : '')?>"
                        id="BlueClock{{$auction->id}}"></em><strong class="primary"
                                                                    id="FriendlyDate{{$auction->id}}">{{$fechafin}}</strong>
                </li>
                <li><em class="icon-material-outline-location-on"></em> {{$port}}</li>
                <li style="display: none" id="HotAuction{{$auction->id}}"><em class="icon-line-awesome-fire red"></em>
                    <strong class="red">¡Subasta caliente!</strong></li>
            </ul>
            ...
            <ul class="task-icons margin-top-20">
                <li>
                    ...
                    <div class="medal-rating {{strtolower($usercat)}}" data-rating="{{$usercat}}"><span
                                class="medal {{$usercat}}-text"></span></div>
                </li>
                <li>
                    <small>Barco</small>
                    <br>
                    <strong><em class="icon-line-awesome-ship"></em> {{ucfirst($auction->batch->arrive->boat->name)}}
                    </strong><br>
            ...
            @if(isset(Auth::user()->id) && $userId==Auth::user()->id)
                <div class="bd-tp-1">
                    ...
                    <a href="#small-dialog-ver-ventas-{{$auction->id}}" class="popup-with-zoom-anim button ripple-effect ico" title="Ver Ventas"
                       data-tippy-placement="top"><i
                                class="icon-feather-dollar-sign"></i> <?=($cantbids > 0) ? ('<span class="button-info">' . $cantbids . '</span>') : ''?>
                    </a>
                    <?php }
                    if($auction->timeline==Constants::FUTURE){?>
                ...
                @include('landing3/partials/pop-up-ventas')
        ...
        <div class="task-listing-bid-inner">
            <div class="task-offers">
                ...
                @if(empty($finished))
                    ...
                    <div id="timer<?=$auction->id?>"
                         class="countdown <?=(isset($finished) && $finished == '&iexcl;Proximamente!') ? 'green ' : ''?>margin-bottom-0 margin-top-20 blink_me <?=(empty($finished)) ? 'timerauction' : ''?>"
                         data-timefin="{{$auction->end}}" data-id="{{$auction->id}}">
            ...
            @if($auction->timeline==Constants::IN_COURSE)
                <div id="OpenerPopUpCompra{{$auction->id}}">
                    <div class="w100">
                        ...
                        <div class="w100 text-center margin-top-5 t14">o puedes <a href="{{$link2}}"
                                                                                   onclick="{{$onclick2}}" <?=$class2?>>realizar
                                una oferta</a></div>
